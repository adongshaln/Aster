...
class AsterUiSmokeTest {
    ...
    @Test fun primaryPagesAndKeyboardRemainUsable() {
        ...
        rule.onNodeWithText("模型分配").assertIsDisplayed()
        screenshot("08-model-settings")
        rule.onNodeWithText("服务连接").performClick()
        screenshot("09-service-settings")
        rule.onNodeWithText("助手偏好").performClick()
        rule.onNodeWithText("让回答更合心意").assertIsDisplayed()
