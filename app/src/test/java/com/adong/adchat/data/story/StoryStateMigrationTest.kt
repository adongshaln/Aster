...
class StoryStateMigrationTest {
    @Test fun populatedOlderDatabasesUpgradeWithoutReclassifyingOldData() {
        ...
        for (version in 1..11) {
            ...
            val old = object : SQLiteOpenHelper(context, StoryDatabase.DATABASE_NAME, null, version) {
                override fun onCreate(db: SQLiteDatabase) {
                    StorySchema.CREATE_STATEMENTS.filterNot {
                        (version == 1 && it.contains(StorySchema.MANUAL_MEMORY_CHANGES)) || (version < 4 && it.contains(StorySchema.CONFLICTS)) || (version < 5 && it.contains(StorySchema.SUMMARY_SOURCES)) || (version < 6 && it.contains(StorySchema.SUMMARY_INPUTS)) || (version < 7 && it.contains(StorySchema.USAGE)) || (version < 8 && it.contains(StorySchema.REWRITES)) || (version<9 && (it.contains("ADD COLUMN mode") || it.contains("ADD COLUMN replacement_input"))) || (version<10 && (it.contains(StorySchema.DISCUSSION_LINKS) || it.contains(StorySchema.MEMORY_DEPENDENCIES))) || (version<11 && it.contains("ADD COLUMN conflicts_with_")) || it.contains("ADD COLUMN attachments_json")
            ...
            try {
                assertEquals(12, upgraded.readableDatabase.version)
