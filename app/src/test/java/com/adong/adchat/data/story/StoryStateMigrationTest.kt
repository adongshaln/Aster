...
class StoryStateMigrationTest {
    @Test fun populatedV1AndV2DatabasesUpgradeWithoutReclassifyingOldData() {
        ...
        for (version in listOf(1, 2)) {
            ...
            val old = object : SQLiteOpenHelper(context, StoryDatabase.DATABASE_NAME, null, version) {
                override fun onCreate(db: SQLiteDatabase) {
                    StorySchema.CREATE_STATEMENTS.filterNot {
                        version == 1 && it.contains(StorySchema.MANUAL_MEMORY_CHANGES)
                    }.forEach { db.execSQL(it.lineSequence().filterNot { line -> line.trim() == "state_key TEXT," }.joinToString("\n")) }
                }
            ...
            db.rawQuery("PRAGMA table_info(${StorySchema.MEMORIES})", null).use { cursor ->
                while (cursor.moveToNext()) assertNotEquals("state_key", cursor.getString(1))
            }
