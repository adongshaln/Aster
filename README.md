...
2.3 重点重做聊天阅读体验，让小说、角色扮演与日常长回复在手机上更接近轻量阅读器，而不是传统聊天气泡堆叠。

当前体验重点包括：

- Aster Logo 用于模型等待状态，采用原地轻跳与空中 90° 旋转的思考动画
- 流式生成采用连续出字与平滑跟随滚动，减少 Token 批次造成的突跳感
- 正文排版收敛到更适合手机长读的字号、行高与段落间距
- 保留对白和明确换行，H1–H6 均可正确解析
- `『术语』` 使用暖棕色强调，适合世界观名词、专有设定与重要概念
- Assistant 回复尾部的技术信息默认收进“详情”，降低对小说阅读的打断
- 底部阅读区域固定使用稳定渲染路径，不再因触摸或静止状态切换不同毛玻璃效果
- 模型切换、新对话等瞬时提示移到顶部区域，并支持点击整条提示立即关闭
- 悬浮输入胶囊、IME 同步上移、顶部提问导航与已有数据兼容性继续保留

更详细的 2.3.0 变更见 [CHANGELOG.md](CHANGELOG.md#230--2026-09-05)。
