...
- 测试覆盖持久化/重启、双击、接受/拒绝/撤销/恢复、冲突注册与决定/撤销的注入失败回滚、旧按钮、来源替换恢复、历史路线、整理任务过期、后续编辑阻止撤销及旧库升级。代码 `12c2a842287c2fd11fc5b8534c9d90880bc67e26` 已通过 [Android Build #104](https://github.com/adongshaln/Aster/actions/runs/34022389634)：单测、Release 编译、固定签名 APK 打包与上传全绿；真实 Gemini 和真机验收尚未执行。
- **边界：只处理结构化属性状态的确定性矛盾。** 自由文本世界观矛盾识别、语义同义键、跨实体知识冲突尚未实现；G2 全部范围和完整故事模式仍未完成。main、应用版本、固定签名不变。


## G3a：未整理正文保留与一致性读取（2026-09-06）

- 实际故事请求改用同一 SQLite 事务读取正式资料、候选与已完成整理的正文版本 ID，避免先读旧资料、后读新 completed 标记的竞态。仅 active+complete Prose assistant 且同路线 organize_prose completed 才算已整理。
- Composer 默认未知覆盖为空集，采取保守保护。从最早未整理回复开始，保留其用户输入和之后连续完整轮次；旧版孤立完整助手正文同样受保护。预留发生在普通资料裁剪之前，保护轮次可超过近期历史软上限，但不突破最终全局硬预算。
- 固定资料、当前输入和待保护正文无法同时装入时，明确阻止发送并提示检查整理失败/关闭情况或精简输入；绝不通过悄悄丢弃未整理旧文解决预算不足。讨论仍不读取正文保护通道，stopped/interrupted 不会因此成为正式历史。
- 只允许较早且已整理的连续轮次按原有规则裁剪；完成标记按 revision 匹配，源版本切换与历史另写的新 ID 均覆盖测试。
- **已整理不等于无损摘要。** completed 只证明整理事务结束；用户撤销资料后既有 completed 标记仍保留，避免自动重建被撤销资料。此轮没有冒称解决摘要覆盖和所有长篇信息遗失问题。
- 测试：软上限为零、保护优先于普通资料、固定资料+正文全局超限、孤立回复、讨论/中断隔离、失败/运行中任务、事务回滚、来源改写恢复与历史路线继承。代码 `eefb1938abfccdd42aebc4d5bb9c61928b213034` 已通过 [Android Build #105](https://github.com/adongshaln/Aster/actions/runs/34022991297)：单测、Release 编译、固定签名 APK 打包上传全绿；尚未进行真实模型/真机验证。
- 后续仍需：摘要的源版本范围及失效依赖、超长整理分段（目前单次来源上限仍 36,000 字符）、真实用量记录。main、版本与签名不变。


## G3b：超长来源分段整理（2026-09-06）

- 实际 organizer 请求按正文 UTF-16 范围连续分段，每段最多 24,000 字符，并为本轮用户输入与前段末尾 512 字符让出空间；总来源输入不超过既有 36,000 上限。优先换行切分，不切断 emoji 代理对。前段末尾明确仅为指代上下文。
- 每个 Job 最多 16 段，超限或用户输入本身过长会在发请求前明确失败，避免无限自动调用。空白段不发模型请求。
- 分段结果校验后以 organizer_chunk 游标保存在既有 snapshots 表，绑定 Job、来源 revision、memoryVersion、范围与输入/协议指纹；失败重试与进程恢复可复用完成分段。版本变化的新 Job 不复用旧分段。
- 正式资料仍只在全来源完成后原子提交，未完成的分段不会进入档案或放行 G3a 保护。提交检查范围连续且覆盖完整来源，并检查各段持久检查点存在且版本/范围一致。
- 按分段次序汇总；同一人物/属性跨段以最终状态为准，实体解析后再次确定次序。同段内部矛盾仍拒绝，模型不可提供 part 序号、来源 ID 或范围。
- 分段范围写入 ChangeSet；这只是本轮提取的来源范围，**不是多轮剧情摘要**。阈值摘要、跨来源摘要依赖与用量记录仍未实现；中途失败会有已消耗调用，不能声称整理每轮固定成本。
- 每段请求前重新检查自动整理开关与当前路线；关闭/切换后按取消处理，保留检查点并停止后续分段调用。
- 最新代码 `2651c4734114dcdee6a1ad2c9106084f889fac27` 已通过 [Android Build #107](https://github.com/adongshaln/Aster/actions/runs/34023962379)：单测、Release 编译、固定签名 APK 打包上传全绿。main、应用版本、签名、数据库 schema 均未改动；尚未进行真实模型/真机验证。


## G3c：跨轮摘要与来源依赖（2026-09-06）

- schema v5 新增 summary_sources，把自动摘要绑定到全部来源 revision；任何来源不再 active+complete 时整条摘要不参与上下文。来源 ID 不随来源删除静默消失，避免只剩部分依赖后错误复活。
- 新增持久 summarize_prose Job，沿用开关、恢复、失败重试、版本提交检查与串行调度；摘要尝试计数与普通提取隔离。保留最近两轮原文，较早已整理正文满足六轮或 12,000 字符时，选择 2–6 轮、合计最多 28,000 字符生成一次摘要。
- 输出仅接受 summary 单字段、最多 3,000 字符；源正文、主观认知与历史状态的边界写入提示词。模型不能提供来源 ID 或范围。摘要、依赖、ChangeSet、memoryVersion 与 Job 完成同事务提交；中途失败或旧版本不生效。
- 真实请求必须带上有效摘要本身，才能省略其覆盖的旧正文；未被有效摘要覆盖的连续后缀继续保留。必需摘要+固定资料超预算则明确失败，不悄悄省略摘要。讨论仍不携带正文历史。
- 整批撤销摘要会保留依赖并使原文重新受保护，不自动重新创建用户撤销的摘要；恢复后重新可用。历史另写从快照复制摘要及全部来源映射，旧快照无摘要依赖时按原格式兼容。
- 测试覆盖阈值与最近原文、摘要前后预算替代、内部来源失效/恢复、版本过期、JSON 白名单、事务回滚、撤销/恢复、历史路线、任务恢复重试以及 v1–v4 有数据升级。代码 `5dfd03aaf39799aac9da5558ce181a5e3b6a6aad` 已通过 [Android Build #108](https://github.com/adongshaln/Aster/actions/runs/34056507735)：单测、Release 编译、固定签名 APK 打包上传全绿。
- **限制仍在：** 摘要是有损压缩，不保证模型语义准确。单轮正文超 28,000 字符暂不纳入跨轮摘要，仍保留原文；本轮未实现摘要再分层压缩、用量统计、真实 Gemini/真机验收，也未关闭其他 G2/G4/G5 缺口。main、正式版本及固定签名不变。


## G3d：分层摘要与可回退的上下文替代（2026-09-06）

- 沿用 summarize_prose 持久 Job，每四份同层、有效、未固定且来源互不重叠的摘要合并一次，输出仍限制 3,000 字符；底层记录不会删除或停用。优先完成可生成的原文摘要，再进行同层合并，避免每轮反复压缩最早历史。
- schema v6 增量新增 summary_inputs，记录所有直接和间接输入摘要的 ID 与内容快照，同时上层摘要保留全部原始正文 revision 依赖。任一祖先被修改、停用、固定、删除或来源失效，上层摘要均不再生效；恢复相同内容和状态后可重新生效。
- 上下文在原一致性事务内选择有效上层摘要，省略它已替代的底层摘要；档案仍保留所有有效记录。上层被撤销后自动回退到底层，并阻止立即自动重建被撤销的相同树。历史另写复制并映射全部摘要依赖，缺失映射继续作为失效依赖保留。
- 摘要输入计划持久保存，执行前/提交前检查现有 Job 与 memoryVersion。资料、全部依赖、ChangeSet、版本和完成状态同事务提交。每次最多四份 3,000 字符摘要；元数据上限为 4,096 个正文来源、1,024 个祖先摘要，超限不丢依赖、不继续合并，仍执行上下文硬预算。
- 新测试覆盖四合一且底层保留、整批撤销回退与不自动重建、修改/固定/恢复、依赖插入失败回滚、旧版本拒绝、历史路线映射及两层压缩后叶子修改传导；旧库升级覆盖 v1–v5。代码 `563ebfa117fe39f97a4f23f6daefa17695f637d3` 已通过 [Android Build #109](https://github.com/adongshaln/Aster/actions/runs/34057553775)：单测、Release 编译、固定签名 APK 打包上传全绿。
- 仍属有损压缩，未验证真实模型语义质量；单轮超 28,000 字符正文的摘要、真实用量记录、完整 G2/G4/G5 与真实 Gemini/真机验收仍未完成。main、正式版本号和固定签名不变。


## G3e：故事调用记录与已返回用量（2026-09-06）

- schema v7 增量新增 story_usage，独立记录正文、讨论、资料整理、摘要的应用调用；保存故事/路线、配置 ID、模型、来源或 Job/分段关联、状态及时间，不保存 API Key、请求头或正文。记录只随故事删除，不随记忆撤销而清除，也不复制到历史路线快照。
- 故事实际三处 API 调用均经同一包装器：请求前持久记录，返回后保存已报告用量；取消时用 NonCancellable 完成状态写入，进程遗留 running 启动时转 interrupted。返回的正文是否完整与资料提交是否成功分别处理，摘要 JSON 错误或旧版本拒绝不会抹掉已发生调用。分段检查点复用不产生新记录；实际重试则独立记录。
- 接口结果增加 providerUsageReported，Chat/Responses 只有同时提供有效输入和输出计数才计为可汇总；明确返回零与未报告区分。聚合使用 SQLite 64 位 SUM；重复结束写入受 running 条件保护，不重复累加。保存失败不丢弃已返回正文，界面明确提示记录未完整保存。
- 档案「变更」新增可展开「故事用量」，按四种用途显示调用次数、返回输入/输出 token 小计、未知/进行中/未完整结束次数及恢复提示，跨当前故事所有路线累计。旧版本历史用量无法补录。
- **计量边界：** 这是 API 返回用量小计，不是完整服务商账单或金额估算。应用调用不等于 HTTP 请求数；底层重试、恢复前失败流、工具中未报告的轮次可能缺失，部分字段/未返回用量保守标未知。未知绝不等于免费。本轮未增加价格表或真实服务商计费核验。
- 测试覆盖 Chat SSE 未报告/缺字段/零、Responses JSON、聚合标志；记录去重、重启恢复、失败重试、跨故事隔离、路线另写不复制、未完整结束仍计返回值与 64 位合计；有数据旧库升级覆盖 v1–v6。代码 `3f20eb292f6a742b78327f76a91dd90a95482c7a` 已通过 [Android Build #110](https://github.com/adongshaln/Aster/actions/runs/34067328800)：单测、Release 编译、固定签名 APK 打包上传全绿。
- main、正式版本及固定签名不变；完整 G2/G4/G5、超大单轮摘要、真实 Gemini 与真机验收仍未完成。


## G4a：正文选段进入讨论草稿（2026-09-07）

- 修订正文输入框保留可选择的字符范围，新增「带入讨论草稿」：选择文字时引用选区，没有选区时引用整段；已手动改动的编辑文本不可冒充原版本引用。未完整结束/停止正文不开放此入口。
- 引用明确标注正文片段、来源消息序号和仅供讨论边界。追加到现有讨论草稿，不自动发送、不创建正文版本、不确认资料或改变 memoryVersion；转入讨论后由用户补充修改想法并发送。
- 引用与讨论草稿保存走 SQLite 原子事务，校验原文 active revision、当前故事/路线及草稿版本；接受尚未异步落盘但更高版本的当前草稿，拒绝旧界面覆盖新草稿。原文或路线已变时要求重新选择。
- 单次引用最多 16,000 字符，合并后草稿最多 40,000 字符；超限拒绝而非截断，校验 UTF-16 边界避免切坏 emoji。正式请求仍受现有全局硬预算约束。
- 测试覆盖逆向选区、保留已有/未落盘草稿、重启恢复、不自动发送或改记忆、旧异步写入拒绝、源版本/路线/草稿过期、超限回滚及 emoji/非完整正文。首次 #111 因测试草稿版本低于数据库初始时间戳而失败，已修正为初始版本之上的递增值并加入保存成功断言。最终代码 `89e7ffece277234a8b21445871e0ca4029e26101` 已通过 [Android Build #112](https://github.com/adongshaln/Aster/actions/runs/34071301709)：单测、Release 编译、固定签名 APK 打包上传全绿。
- **G4 尚未完整完成：** 本轮交付选段进入讨论这一入口；模型重写候选、独立预览、采用时的原文/资料版本检查及历史位置重写上下文仍待实现。引用是固定文字，不是会跟随原文更新的实时链接。main、正式版本、签名及 schema 未变。


## G4b：最新正文的模型重写候选、预览与采用（2026-09-07）

- 新增 schema v8 story_rewrites，候选独立于正式消息/资料，记录来源 revision、路线、memoryVersion、明确修改要求、服务/模型、候选正文和状态；生成中定期保存，停止/失败/未完整输出保留片段但不能采用，进程恢复将生成中候选标为 interrupted。重新打开可预览该消息最近一次候选。
- 修订窗口新增「让模型重写这一段」，填写明确修改要求后生成，先预览再「采用为新版本」。支持当前路线最新正文；后续完整讨论不阻止重写和版本恢复，但新增正文输入/回复、任意正在流式生成内容、旧来源或路线切换均阻止采用。
- 重写上下文复用正式正文预算组织器：固定约束必须保留，剔除待替换正文派生的非固定资料/摘要；原文与原请求明确作为修改素材，只有用户本次明确输入的修改要求进入请求，不自动复制讨论消息、Proposal 或推断。输入超硬预算明确失败；不开放历史中段的自动重写。
- 请求通过已有用量包装器计入正文调用，禁用工具。完整输出才成为 ready；采用时重新检查路线、原文 revision、memoryVersion 与正文末尾条件，版本切换、来源记忆失效、版本增长和候选 adopted 状态处于同一 SQLite 事务，失败全部回滚。双击不能重复采用。采用后安排新版本资料整理。
- UI 支持停止、候选复制/预览、重新生成；候选不会自动成为事实。当前只展示最近一次候选，不是候选版本浏览器；尚未把讨论结论自动归纳成可采用的修改单。模型不保证逐字保留未修改部分，仍需用户预览。
- 测试覆盖预览持久化且不改正文/版本、讨论后采用与恢复、资料/正文后续变化拒绝、停止/失败/重启候选门禁、采用失败事务回滚、旧正文派生资料与讨论隔离、固定约束和硬预算；旧库升级覆盖 v1–v7。首次 #113 的旧测试仍假定任何后续讨论均禁止恢复，已更新为允许后续讨论并继续验证后续正文拒绝。最终代码 `aa7f03291c8b07cf0edab1f1b3144cecb8e1b0f6` 已通过 [Android Build #114](https://github.com/adongshaln/Aster/actions/runs/34073536441)：单测、Release 编译、固定签名 APK 打包上传全绿。
- G4 完整范围仍缺历史中段快照上下文重写和讨论修改单联动；完整 G2/G5、超大单轮摘要、真实模型/真机验收仍未完成。main、正式版本和固定签名不变。


## G4c：历史快照重写与旧输入修改（2026-09-07）

历史中段模型重写读取生成前物化快照，校验前缀版本；正式采用创建独立有效路线，原后续保留。可修改对应原始用户输入，只有新路线中的副本变化。schema v9 保存候选 mode/replacement_input；上下文从快照恢复资料、摘要依赖和整理覆盖，不读取之后的资料/剧情。无快照或前缀变化仍明确拒绝。新增未来信息隔离和旧输入改写测试，待 CI。本轮继续推进其余清单，不将此保存点宣称为完整交付。


## G4d / G5a：讨论应用、编辑后确认与来源复核（2026-09-07）

讨论回复新增明确应用入口：编辑建议后送入后续正文草稿、选择对应原段准备重写、确认世界设定或作者计划；不自动发送。相关回复显示待定候选入口，档案支持编辑后采用。支持精确且唯一的「采用：完整候选内容」命令并记录用户决定 revision；无法唯一匹配则本地澄清，不调用模型代替用户裁决。普通肯定语不会自动采用。

schema v10 持久保存选段到讨论的来源联系，以及确认资料对原文版本的依赖；连续讨论沿用最近引用来源。原文重写后相关资料退出正式上下文并列入「待复核」，恢复原文可重新有效。快照与历史另写复制映射依赖。旧数据不猜测来源，不进行破坏性迁移；缺失引用的历史自然语言关联不能凭空补齐。待复核条目可停用；修正后可手动新增独立资料。本提交待 CI，后续继续补长篇与冲突路径。

## G3f · 超长单条正文的分段摘要（2026-09-07）

- 对已整理且离开最近两轮的超长正文，按最多 16 段生成局部摘要，再按四合一合并；单任务最多 21 次请求。
- 每个节点保存输入指纹、资料版本和完整 JSON，失败恢复复用已完成节点；版本或来源失效拒绝读取和提交。
- 局部结果仅为工作检查点，最终根摘要同原子事务写入完整来源依赖后才释放原文保护。调用仍逐次记入用量。
- 新增超长单来源、请求长度、失败恢复与版本变化测试。本提交待 CI 验证。

## G2b · 自由文本设定疑似冲突（2026-09-07）

- 整理协议允许客观世界/人物设定以 `contradicts` 逐字引用旧内容。应用只接受同路线、有效、唯一的资料匹配，自行绑定 ID；模糊或不存在的引用整批拒绝。
- schema v11 增量保存矛盾对象与原内容。双方保留，正文请求阻止，讨论中标明待裁决；沿用可撤销的用户选择事务与来源展示。
- 路线分支映射冲突引用，旧对象修改/停用、来源失效后重新计算。主观怀疑和普通状态变化不得使用此字段。
- 这是模型辅助的疑似矛盾识别，不承诺穷尽语义矛盾；识别质量需真实模型验证。新增解析/数据库/上下文/选择撤销/路线测试，待 CI。

## G5b · 关联决定复核与独立设定重新适用（2026-09-07）

- 档案提供“复核并确认”：编辑新的判断后原子停用当前路线旧关联记录并新增独立资料，各自写入审计；旧证据保留。来源复原不会覆盖用户新的决定。
- 历史另写之后，旧路线后续关联决定列为待复核，未进入新正文；旧路线独立设定可逐项检查并采用。采用只写新路线，映射人物实体，不自动携带旧后续事件。
- 已存在的同类同内容或已采用来源去重，重复点击与跨路线过期操作拒绝。修改设定不自动重写历史正文。
- 明确但指向不明的简短选择（如“这个不错”“采用这个”）在有候选时本地澄清，不整批采用；任意自然语言仍不能被当作自动管理授权。
- 新增复核事务失败回滚、旧来源恢复、新旧路线隔离、重新采用去重测试。待 CI。
...
## 试用修复：输入法关闭后输入框不收起（2026-09-07）

故事 Composer 原先仅跟随焦点，IME 收起不自动失焦。现在沿用普通聊天的 IME 当前/动画目标监听，仅在本次焦点已见到键盘后响应收起，避免首次聚焦被提前取消；收起时只显示一行，草稿内容不变。讨论和正文共用修复。待 CI 和用户手机复测。

## 试用补齐：图片与文件输入（2026-09-07）

- 普通聊天新增文件入口；故事输入框新增图片/文件菜单、图片预览和移除。TXT/Markdown/JSON/CSV 等文本、DOCX 正文和 PDF 文字层导入可检查的草稿，随后通过既有 Chat/Responses 文本请求发送。
- 文件最多 10 MB，导入与草稿合计最多 40,000 字符；不支持扫描 PDF OCR 或旧二进制 DOC，明确报错，拒绝静默截断。PDF 依赖按 [PdfBox-Android 官方说明](https://github.com/TomRoush/PdfBox-Android) 接入。
- 故事图片每条最多 4 张，保存应用内副本；schema v12 在消息版本与工作区草稿中持久化元信息。上下文按图片预留预算，实际请求使用原有 image_url/input_image 编码。讨论图片不自动带入正文；图片识别需所选模型支持视觉。
- 路线快照保留图片引用及草稿，重写保留原请求图片；删除故事时清理对应图片。输入框按实际高度给消息列表留空间。
- 新增文字解码/超限/DOCX 提取，以及 SQLite 图片持久化、两区隔离、路线映射测试。待 CI 和手机选取/实际网关验证。
