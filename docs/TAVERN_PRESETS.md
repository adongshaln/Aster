...
- 显示正则清理前分别提取 `konatan_planning~`（预设思考）和 `think` / `thinking`（思考文本），各自保留流式与未完整结束状态；原始回复可直接展开核对
...
思考片段内以对应结束标签作为边界，未闭合的 Markdown 代码块不会延伸到后续正文。普通正文中代码围栏里的标签示例保持原样；思考文本内的规划草稿仍属于该思考文本，不会混入外部的预设思考。

「思考文本」与「预设思考」仅展示模型在回复正文中实际返回的文本，不代表可以读取供应商隐藏的内部推理。没有识别到这些片段时，界面明确说明；不能据此断言模型没有思考。思考片段会独立保留，不再被正文显示正则隐藏；请求侧正则和原始消息不受影响。
